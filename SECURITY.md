...
- The Scorecard `Signed-Releases` check is not applicable until the first version is tagged. `.github/immutable-releases.json` records the enabled repository policy, and the release workflow requires attestations for every archive, SBOM, and checksum file.
