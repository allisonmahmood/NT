...
- A parent physically containing a registered nested worktree is refused if that
  child is omitted or its removal fails; the error names the protected child.
- Nested parent+child both named in one batch → remove children first, then the
  parent, with exit 0 when all removals succeed.
